// api/normas-regulamentadoras/[numero].js
// Base de dados das NRs (mesma do arquivo anterior)
const nrsDatabase = {
  "NR-01": {
    numero: "NR-01",
...
export default function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const nrKey = numero.toUpperCase();
    const nr = nrsDatabase[nrKey];
    
    if (!nr) {
      return res.status(404).json({
        success: false,
        error: "NR não encontrada"
      });
    }
